...
class Memento {
 public:
  Memento() {}
  Memento(QString pathToTexture, GroupSettings stateSets,
          Camera3D stateCamera) {
    setState(pathToTexture, stateSets, stateCamera);
  }

 private:
  friend Controller;
  GroupSettings getStateSets() { return stateSets_; }
  Camera3D getStateCamera() { return stateCamera_; }
  QString getStateTexture() { return pathToTexture_; }
  void setState(QString pathToTexture, GroupSettings stateSets,
                Camera3D stateCamera) {
...
class Controller {
 ...
 public:
  ...
  const unsigned &get_projection_sets() { return settings_.get_projection(); }
  const unsigned &get_line_type_sets(const unsigned &indexSetting = 0) {
    return settings_.get_lineType_index_setting(indexSetting);
  }
  const float &get_line_size_sets(const unsigned &indexSetting = 0) {
    return settings_.get_lineSize_index_setting(indexSetting);
  }
  const QVector4D get_line_color_sets(const unsigned &indexSetting = 0) {
    std::array<float, RGBA::N> tmp =
        settings_.get_lineColor_index_setting(indexSetting);
    return QVector4D(tmp[RGBA::RED], tmp[RGBA::GREEN], tmp[RGBA::BLUE],
                     tmp[RGBA::ALPHA]);
  }
  const unsigned &get_vert_type_sets(const unsigned &indexSetting = 0) {
    return settings_.get_vertType_index_setting(indexSetting);
  }
  const float &get_vert_size_sets(const unsigned &indexSetting = 0) {
    return settings_.get_vertSize_index_setting(indexSetting);
  }
  const QVector4D get_vert_color_sets(const unsigned &indexSetting = 0) {
    std::array<float, RGBA::N> tmp =
        settings_.get_vertColor_index_setting(indexSetting);
    return QVector4D(tmp[RGBA::RED], tmp[RGBA::GREEN], tmp[RGBA::BLUE],
                     tmp[RGBA::ALPHA]);
  }
  ...
  const QVector4D get_surfaces_color_sets(const unsigned &indexSetting = 0) {
    std::array<float, RGBA::N> tmp =
        settings_.get_surfaceColor_index_setting(indexSetting);
    return QVector4D(tmp[RGBA::RED], tmp[RGBA::GREEN], tmp[RGBA::BLUE],
                     tmp[RGBA::ALPHA]);
  }
  ...
  const QString get_texture_path_sets(const unsigned &indexSetting = 0) {
    QString texturePath = QString::fromUtf8(
        settings_.get_texture_path_index_settings(indexSetting).c_str());
    return texturePath;
  }
  ...
  void set_line_type_sets(const unsigned &lineType,
                          const unsigned &indexSetting = 0) {
    settings_.set_lineType_index_setting(lineType, indexSetting);
  }
  void set_line_color_sets(const float &r, const float &g, const float &b,
                           const float &a, const unsigned &indexSetting = 0) {
    settings_.set_lineColor_index_setting(r, g, b, a, indexSetting);
  }
  void set_line_size_sets(const float &lineSize,
                          const unsigned &indexSetting = 0) {
    settings_.set_lineSize_index_setting(lineSize, indexSetting);
  }
  void set_vert_type_sets(const unsigned &vertType,
                          const unsigned &indexSetting = 0) {
    settings_.set_vertType_index_setting(vertType, indexSetting);
  }
  void set_vert_color_sets(const float &r, const float &g, const float &b,
                           const float &a, const unsigned &indexSetting = 0) {
    settings_.set_vertColor_index_setting(r, g, b, a, indexSetting);
  }
  void set_vert_size_sets(const float &vertSize,
                          const unsigned &indexSetting = 0) {
    settings_.set_vertSize_index_setting(vertSize, indexSetting);
  }
  void set_bg_color_sets(const float &r, const float &g, const float &b,
                         const float &a) {
    settings_.set_bgColor(r, g, b, a);
  }
  void set_surfaces_color_sets(const float &r, const float &g, const float &b,
                               const float &a,
                               const unsigned &indexSetting = 0) {
    settings_.set_surfaceColor_index_setting(r, g, b, a, indexSetting);
  }
  void set_draw_type_sets(const unsigned &drawType,
                          const unsigned &indexSetting = 0) {
    settings_.set_drawType_index_setting(drawType, indexSetting);
  }
  void set_texture_path_sets(const QString &texturePath,
                             const unsigned &indexSetting = 0) {
  ...
  /*-----CAMERA OPTIONS------*/
  void translate_camera(const QVector3D &t) { camera_.translate(t); }
  void rotate_camera(const QQuaternion &r) { camera_.rotate(r); }
  void scale_camera(const float &s) { camera_.scale(s); }
  const QMatrix4x4 &get_view_matrix() { return camera_.getViewMatrix(); }

  /*-----OBJECTS OPTIONS------*/
  bool have_objects() { return !objects_.isEmpty(); }
  unsigned get_sum_objects() { return objects_.sumObjects(); }

  bool add_object(const QString &filePath) {
    QString texturePath =
        QString::fromUtf8(settings_.get_texture_path_index_settings().c_str());
  ...
  void delete_all_objects() { objects_.deleteAllObjects(); }
  /*----translate----*/
  ...
  const QMatrix4x4 get_model_matrix_object(const unsigned &indexObj = 0) {
    Adapter adapter = Adapter::getInstance();
    return adapter.adapt(objects_.get_model_matrix_object(indexObj));
  }
  ...
  void add_texture_to_object(const QString &pathTexture,
  ...
  void add_texture_to_object(const unsigned &indexObj = 0) {
    QString texturePath = QString::fromUtf8(
        settings_.get_texture_path_index_settings(indexObj).c_str());
  ...
  const QVector4D getLightPosition(const unsigned &indexSetting = 0) {
    std::array<float, RGBA::N> tmp =
        settings_.get_lightPosition_index_light(indexSetting);
    return QVector4D(tmp[RGBA::RED], tmp[RGBA::GREEN], tmp[RGBA::BLUE],
                     tmp[RGBA::ALPHA]);
  }
  ...
  const QVector4D getSpecularColor(const unsigned &indexSetting = 0) {
    std::array<float, RGBA::N> color =
        settings_.get_lightSpecularColor_index_light(indexSetting);
    return QVector4D(color[RGBA::RED], color[RGBA::GREEN], color[RGBA::BLUE],
                     color[RGBA::ALPHA]);
  }
  ...
  void setLightColorGreen(const float &green,
                          const unsigned &indexSetting = 0) {
  ...
  /*-----LIGHT STATUS------*/
  const bool &getLightStatus() { return settings_.get_lightStatus(); }
  void setLightStatus(const bool &status) { settings_.set_lightStatus(status); }

  /*-----LIGHT SHADE------*/
  const bool &getLightShade() { return settings_.get_lightShade(); }
  void setLightShade(const bool &shade) { settings_.set_lightShade(shade); }

  /*---WINDOW SIZE SETTINGS---*/
  const float &get_windowWidth_sets() { return settings_.get_window_width(); }
  const float &get_windowHeight_sets() { return settings_.get_window_height(); }
  ...
  void save_settings() { settings_.save(); }
  void save_camera() { camera_.save_camera(); }
